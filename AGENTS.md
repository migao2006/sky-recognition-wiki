...
- 單一作者的縮寫、錯字或排版習慣只能作為搜尋佐證，不得直接取代通用玩家名稱；「耳尾組」「三件套」「套組」等集合詞只能綁定已確認的完整成員並作為群組搜尋別名，不得成為任何單件 `displayName` 或 `saleName`。
- 付費物品由英文名稱產生的自動翻譯不得直接進入出售文案；確認過的玩家稱呼須以官方 GUID 寫入 `reviewed-iap-player-names.json`，舊直譯只可保留為搜尋別名。短稱可以使用，不得為了補足類型而改回逐字翻譯。
- `saleName` 只能保存已有社群文案、玩家別名或其他可追溯依據的 2～6 字出售短名，且必須以官方 GUID 綁定；它只供帳號分享摘要使用，不得取代衣櫃 `displayName`、搜尋名、備份或圖片輸出。短名撞名時須保留最短可辨識系列前綴，不能靜默去重或任意截字。分享摘要以物品本身的顯示字數換行，系列標題不占物品列額度；不得用補空白、裁字或重複符號製造假對齊。
- 分享摘要的帳號標題必須直接復用估價結果的最早畢業進度季 `startSeasonSlug`、`breakClass` 與 `salePackageTier`，不得從摘要字串猜測。`salePackageTier` 以去除同包多件後的唯一付費禮包數判定：0–59 少禮、60–89 中禮、90 以上多禮；免費限定、週年收藏與季節畢業禮不計入。此門檻依指定交易文案庫中 60、70、90 與百禮的實際用法維護。估價資料的 `paid_package_count`、內部 `packageTier`、禮包加值上限與帳號類型也必須使用相同的唯一真實禮包語意；付費 catalog item 數只能作診斷，不得拿來跨級。內部 `packageTier` 只負責既有價格校準，不得拿來生成摘要標題。部分畢業可作年代起點但不算斷季；完全沒有畢業進度時不得杜撰斷季。資源數量只列帳號已填寫且大於零的白蠟、愛心、昇華蠟與副卡。
- 套組詞只能作為每個已確認成員的共用搜尋別名，不得建立虛假套組物品，也不得在分享摘要中輸出成單件物品。
- Fandom 與 BWiki 蒐集結果只是候選證據；Fandom 標記國際服、BWiki 標記中國服，禁止直接跨服合併。頁面文字或 OCR 名稱必須再以官方 GUID、原始類型與 SkyGame-Data 身分核對，才能寫入顯示名、禮包或估價規則。
- Wiki 核對報告本身不得寫回正式資料。Fandom 只有圖示檔名與官方資料完全一致時才可確認國際服物品身分；BWiki 即使解析到共用 GUID，也只能標為中國服參考。套裝名稱同時出現在多種圖片類型時須保留為待拆解項目，不得硬配到其中一件。
- Wiki 模組、圖片檔名或 OCR 中出現 `iap`／禮包字樣，只能作為候選線索，不能單獨判定付費。付費身分必須由 SkyGame-Data 的 package／price 關聯或頁面明確售價交叉確認；活動代幣、蠟燭兌換與付費外觀相近時尤其不得混用。
- 交易用語大量同步須優先使用同衣櫃分類內「來源原名完全一致且唯一」的 GUID；順序只能補配兩個已確認錨點之間筆數相同且不超過 16 件的區段，並仍須通過名稱相似門檻。Fandom 精確 icon 標籤只可補搜尋別名；付費主名稱必須另有台灣玩家用語，不得以 Wiki 翻譯取代。
- 季節縮寫、錯別字與交易簡稱只能放入 `seasonSearchAliases` 供搜尋使用，不得取代 `seasonZh` 的標準名稱。
- 「全圖、熱門復刻、斷季、微斷」等帳號狀態不是物品名稱；無法唯一對應 GUID 的詞必須保留現況並標記未採用。

### Catalog、備份與估價發布限制

- 標題解析與行情稽核必須共用有效標題選取規則，略過空白及文件分頁標記；物品／套組名稱後接「禮包號／禮包簡」不能單獨證明起季。全文核對更改起季時，同步檢查季節進度與衍生斷季欄位；只有季卡、修改取得的早期物品不能冒充一般畢業進度。

- 參考校準可採用不完整文案：audit 的標題起季 fallback 使用 0.45 證據係數，缺少綁定、資源或完整 GUID 不得直接排除整筆；未知欄位只略過相應維度。禮包下界／區間保存為 `paid_package_min/max`，不得將 60+ 填為精確 60；跨內部級距則保留未知，否定或矛盾數量不得猜配。相關來源欄位必須參與穩定去重鍵及證據簽章；新增標題／範圍欄位的舊正式證據需核對後重簽 v2，未含這些欄位的 v1 保持相容。細節見 README「不需完整 GUID 的標題行情」。

- 行情蒐集可主動上網搜尋，接受標題明確提供的起季、斷季程度、簡號與禮包級距，不要求逐筆還原完整 GUID。標題行情與完整衣櫃重播是不同證據路徑；不得為補齊 predictor 杜撰季節進度、綁定、資源或物品。模糊季名、多季衝突及只有季節物品的標題保留未知。
- 標題行情必須分開來源、伺服器、原幣與價格類型，賣家「少／中／多禮」不得自動當作精確禮包數。整號售價已含禮包，不能當季節裸價後再加完整禮包溢價；先比較同型帳號的禮包級距差額，少禮不能視為零禮，未知綁定等混雜因素必須明示。未通過完整驗證的標題報告只能作探索性參考，不降低下列正式驗證門檻。
- 曾提供人工答案的測試帳號，應用 `partition-valuation-evaluation.mjs` 依帳號、貼文與快照識別連同副本從校準來源隔離；不得靠姓名或價格比對。原始資料保留在私人工作目錄，不提交 Git。已看過答案的案例即使後來隔離也只能稱回歸測試，不能稱從未見過的盲測或代替正式 holdout。
...
- 由帳號備份建立正式估價樣本時，必須人工確認衣櫃完整、使用目前 v4 備份、明確保存七個平台綁定與四項資源。衣櫃確認只能由衣櫃頁的使用者核取寫入，v1–v3 匯入後即使補產生身分也不得視為當時已確認；v4 的私密隨機 UUID v4 是唯一可用的長期匿名身份，CLI 參數只能交叉核對、不得覆蓋。不得用會隨衣櫃、綁定或資源變動的快照雜湊冒充帳號身份。帳號身份與快照雜湊必須分開，且同一資料集持續使用同一私密 salt；來源文字等所有會改變納入資格的欄位必須一併簽章，完全相同的快照只能計一次，避免重複帳號及 holdout 洩漏。
