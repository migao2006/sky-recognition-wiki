...
已由網站匯出的完整帳號備份可用 `npm run prepare:valuation-sample -- --backup <備份.json> --price-twd <人工或成交價> --account-id <私密穩定 UUID v4> --inventory-complete yes --evidence-kind professional_estimate --out work/sample.jsonl` 產生可重播的匿名樣本。執行前必須在本機設定至少 32 字元、且同一資料集持續沿用的 `VALUATION_HASH_SALT`；`--account-id` 必須是同一帳號跨次採樣都不改變的私密 UUID v4，可用 PowerShell `New-Guid` 產生，不可使用姓名、信箱或社群帳號。工具只接受 `sold` 或 `professional_estimate`；`--inventory-complete yes` 代表已人工確認備份包含完整衣櫃，且備份必須是目前 v3、明確保存七個平台的有效綁定狀態、不得包含未知或被略過的 GUID，白蠟、愛心、昇華蠟、副卡四項資源也都必須明確填入整數，空白不會被當成 0。輸出強制留在 `work/`；穩定帳號指紋、salt 命名空間與個別快照雜湊分開以 HMAC 產生，七平台、四資源、價格、來源分組、日期、季節進度、分類及 predictor 也會一併簽章。audit 與 validator 只接受能由相同本機 salt 驗證的完整證據，任何事後改價、改社團、改權重來源或排除狀態都會失敗；帳號名稱、私密 UUID、備註與原始檔案路徑不會寫入樣本。
