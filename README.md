...
目前校準資料：2026-09-10 已完成另一組 18 筆「梵谷有卡」原文核對，移除誤記的完整畢業，補回漏季與原文部分比例，取消由清單件數猜出的唯一禮包數，並分開刊登價與秒價。三筆包含徽章／白卡且無法拆出帳號本體價格的混合總價保留原始紀錄，但不參與模型；資料不完整本身仍不是排除理由。更新後為 446 筆來源、396 筆有效帳號、353 筆探索校準、43 筆舊式保留組，Drive 有效列 111 筆。相較上一版，歸屬基準中位數 74,400 → 74,300、魔法 16,300 → 16,400；其餘季節基準中位數與分類乘數不變，部分上下限及有效權重有調整。仍為 `unvalidated`，不代表已通過完整衣櫃估價驗證。以下保留先前各批核對時的統計。
