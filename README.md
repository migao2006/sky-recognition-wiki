...
當未提供明確起季而須從 `season_progress` 推斷時，audit 依實際非零進度判定，支援文字比例與 `{ selected, expected }`；零取得、無效比例或空物件不會只因存在季節鍵而成為起季。既有 `start` 標記仍相容，其他已知行情欄位也不因單一進度無效而整筆丟棄。
