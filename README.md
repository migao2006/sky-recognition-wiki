...
私人文案重播會將獨立「實體徽章／實體 STAR 徽章」標題下、到下一空白行為止的清單隔離，不拿來辨識衣櫃 GUID、綁定或資源。輸出 `physical_collectibles_present` 及缺漏標記 `physical_collectibles_scope`，保留未拆價的不完整性；此規則不自行斷定合售或自動排除市場資料，也不把單純「GG 綁全出」後的清單猜成徽章。正常衣櫃中的「大傘」別名仍保留。
...
季節比例報告、標題行情報告與私人文案重播摘要共用未加權 type 7 分位數算法；偶數筆中位數取中間兩筆平均，避免取偏低值而產生不一致的季節倍率或重播價差統計。物品數中位數因此可能是小數，並非建立半件物品。此統計修正不改逐筆 GUID 或估價。比例報告也遵守共用人工排除標記（`exclude_from_model`／`exclusion_reason`），不能因舊候選旗標仍為 true 而重新採用合售或已確認不適用的資料。這不改正式模型的證據加權分位數，也不把不完整衣櫃直接列為人工排除。

明列 `【畢業季節】：表演季，極光季，狂歡季` 的清單可取官方時間順序最早的已列季節作參考起點，清單順序不限；不補中間缺季、不推定無斷或完整衣櫃。混入季卡、物品、未知季名或重複清單標題仍待查。「畢業禮多禮號／不是姆明畢業多禮號」不因附帶禮包級距而升格為起季證據，已知禮包級距仍可保留。
