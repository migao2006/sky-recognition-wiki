...
部分畢業扣分只作用於起始季的完整畢業價格基準；後續季節進度由斷季分類處理，不再因從零件補選第一件畢業禮而追加扣款。相同斷季級距內，後續部分畢業與全畢可能同價，尚未以成交資料校準單件溢價。Predictor schema v4 保存此新語意；舊 v3 必須由原帳號重新計算與簽章，不能只改版本號，仍可作不完整行情參考。這不是備份格式升版，固定 holdout 種子不變。固定 v2 baseline 僅開放比較路徑，依其價格帶及完整進度重算舊版全季部分畢業扣分；不能作新版候選，也不沿用 v4 的起季扣分值冒充舊算法。
