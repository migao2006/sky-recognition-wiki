...
季節比例報告與標題行情報告共用未加權 type 7 四分位算法；偶數筆中位數取中間兩筆平均，避免前者仍取偏低值而產生不一致的季節倍率。比例報告也遵守共用人工排除標記（`exclude_from_model`／`exclusion_reason`），不能因舊候選旗標仍為 true 而重新採用合售或已確認不適用的資料。這不改正式模型的證據加權分位數，也不把不完整衣櫃直接列為人工排除。
