...
已由網站匯出的完整帳號備份可用 `npm run prepare:valuation-sample -- --backup <備份.json> --price-twd <人工或成交價> --evidence-kind professional_estimate --out work/sample.jsonl` 產生可重播的匿名樣本。執行前必須在本機設定至少 32 字元的 `VALUATION_HASH_SALT`；工具只接受 `sold` 或 `professional_estimate`，備份必須確認所有綁定、不得包含未知或被略過的 GUID，且白蠟、愛心、昇華蠟、副卡四項資源都必須明確填入整數，空白不會被當成 0。輸出強制留在 `work/`；帳號指紋以 HMAC 產生，帳號名稱、備註與原始檔案路徑不會寫入樣本。
